...
Result: ~254 k triangles with real silhouettes on every pocket and hole.

## Textures

Base colour comes from the **photographs themselves**. The front panel and
back plate were perspective-rectified to true orthographic scale, de-lit by
dividing out a heavily blurred copy of themselves, masked to the device
outline, and resampled into the atlas. So the aluminium tone, the scuffs
along the bottom edge and the complete laser etching on the back — the TE
logo, the FCC text, the CE mark, the charge and battery icons — are the real
article rather than a redrawing.

Procedural detail fills in what a photograph cannot give: the bead-blast
micro-relief, machining rings in the pocket floors, the knurl on the slider
knobs, and the side faces, for which no straight-on reference exists.

The normal map carries only detail that is *not* in the mesh, so nothing is
double counted.
